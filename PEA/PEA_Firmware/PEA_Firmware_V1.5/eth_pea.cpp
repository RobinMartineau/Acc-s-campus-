...
const char* serverHost = "api.campus.local";
 ...
 * 
 * HTTPGetIDSalle
 * Fontion qui permet de lier l'adresse MAC de la PEA à un identifiant dans la base de donnée
 * 
 ============================================================================================*/
int httpGetIDSalle(const String& numSalle)
{
  ...
  if (client.connect(serverHost, SERVER_PORT)) {
    ...
    if (httpCode == 200) {
      ...
      if (!err && doc["detail"][0]["msg"]) {
        Serial.print("Erreur 422 : ");
        Serial.println(doc["detail"][0]["msg"].as<String>());
      } else {
        Serial.println("Erreur 422 : Détail non lisible");
      }
    } else {
      Serial.print("Code HTTP inattendu : ");
 ...
 * 
 * addEquipement
 * Fontion qui permet d'ajouter l'equipement à la base de donnée en lui envoyant l'id, l'adresse MAC et le type via cette requete JSON.
 * 
 * {
 * "adresse_mac": "string",
 * "type": "PEA ou BAE",
...
void addEquipement(int idSalle)
{
  char macStr[18];
  sprintf(macStr, "%02X:%02X:%02X:%02X:%02X:%02X",
          mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
  ...
  doc["adresse_mac"] = macStr;
  doc["type"] = "PEA";
  doc["id_salle"] = idSalle;
  ...
  Serial.println("Connexion au serveur pour enregistrement équipement…");
  if (client.connect(serverHost, SERVER_PORT)) {
    client.println("POST /equipement/ HTTP/1.1");
    ...
    Serial.println("Échec de connexion pour l'enregistrement de l'équipement");
  }
 ...
 * 
 * DeleteEquipement
 * Fontion qui permet la suppresion de l'ID dans la 
 * 
 ============================================================================================*/

 void deleteEquipement(int idEquipement)
{
  digitalWrite(ETH_CS, LOW);
  delay(50);
