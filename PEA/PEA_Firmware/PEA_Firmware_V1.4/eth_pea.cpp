...
const char* SERVER_HOST = "api.campus.local";
EthernetClient baseClient;
//SSLClient tlsClient(baseClient, TAs, TAs_NUM);
...
String sendHttpPost(String uidHex)
{
  ...
  if (tlsClient.connect(SERVER_HOST, SERVER_PORT)) {
    ...
    tlsClient.println("POST /pea/acces/badge HTTP/1.1");
    tlsClient.println("Host: api.campus.local");
    tlsClient.println("Content-Type: application/json");
    tlsClient.print("Content-Length: ");
    tlsClient.println(jsonBody.length());
    tlsClient.println("Connection: close");
    tlsClient.println();
    tlsClient.print(jsonBody);
...
String sendHttpPostPassword(String code)
{
  ...
  if (tlsClient.connect(SERVER_HOST, SERVER_PORT)) {
    ...
    tlsClient.println("POST /pea/acces/digicode HTTP/1.1");
    tlsClient.println("Host: api.campus.local");
    tlsClient.println("Content-Type: application/json");
    tlsClient.print("Content-Length: ");
    tlsClient.println(jsonBody.length());
...
int httpGetIDSalle(const String& numSalle)
{
  ...
  Serial.println("Connexion au serveur…");
  if (tlsClient.connect(SERVER_HOST, SERVER_PORT)) {          // serverPort = 8000
      //---------------   ENVOI DE LA REQUÊTE GET   -----------------
      tlsClient.print(F("GET /salle/"));
      tlsClient.print(numSalle);
      tlsClient.println(F(" HTTP/1.1"));
      tlsClient.println(F("Host: api.campus.local"));
